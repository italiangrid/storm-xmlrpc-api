...
public class TCheckSumType {
    ...
    public TCheckSumType(String chkType)  throws IllegalArgumentException
    {
        if ((chkType == null) || (chkType.length() == 0))
        {
            throw new IllegalArgumentException("Unable to construct the TCheckSumType. Invalid chkType=" + chkType);
        }
    ...
    public static TCheckSumType decode(Map<String, Object> param, String name) throws IllegalArgumentException
    {
        String type = (String) param.get(name);
        if (type != null)
        {
            return new TCheckSumType(type);
        }
        else
        {
            throw new IllegalArgumentException("Unable to decode the object, no valid \'" + name
