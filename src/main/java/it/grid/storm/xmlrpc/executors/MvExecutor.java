...
public class MvExecutor
{
    ...
    public static RequestOutputData execute(synchcall storm, String userDN, List<String> userFQANS, String fromSurl,
            String toSurl) throws ApiException
    {
        if (storm == null || userDN == null || userFQANS == null || userFQANS.isEmpty() || fromSurl == null
                || toSurl == null)
        {
            throw new IllegalArgumentException("Unable to call mv command. Received null arguments: storm="
                    + (storm == null ? "null" : "not null") + " userDN=" + userDN + " userFQANS=" + userFQANS + " fromSurl=" + fromSurl + " toSurl=" + toSurl);
        }
        Map<String, Object> parameters;
        try
        {
            parameters = MvEncoder.getInstance().encode(userDN, userFQANS, fromSurl, toSurl);
    ...
    private static RequestOutputData doIt(synchcall storm, Map<String, Object> parameters) throws ApiException
    {
        ...
        {
            throw new ApiException("Unable to perform mv call. XmlRpcFault: " + e.getMessage());
        }
        try
        {
        ...
        {
            throw new ApiException("Unable to decode mv call output. IllegalArgumentException: "
                    + e.getMessage());
