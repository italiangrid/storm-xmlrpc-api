...
public class RmExecutor {
	...
	public static RequestOutputData execute(synchcall storm, String userDN,
		...
		if (storm == null || userDN == null || userFQANS == null
			...
			throw new IllegalArgumentException(
				"Unable to call rm command. Received null arguments: storm="
					+ (storm == null ? "null" : "not null") + " userDN=" + userDN
					+ " userFQANS=" + userFQANS + " surls=" + surls);
		}
		Map<String, Object> parameters;
		try {
			parameters = SurlArrayRequestEncoder.getInstance().encode(userDN,
				userFQANS, surls);
		} catch (IllegalArgumentException e) {
			throw new ApiException(
				"Unable to encode rm parameters. IllegalArgumentException: "
					+ e.getMessage());
		}
	...
	public static RequestOutputData execute(synchcall storm, List<String> surls)
		throws ApiException {
	...
	private static RequestOutputData doIt(synchcall storm,
		...
		try {
			...
			throw new ApiException("Unable to perform rm call. XmlRpcFault: "
				+ e.getMessage());
		}
		try {
			...
			throw new ApiException(
				"Unable to decode rm call output. IllegalArgumentException: "
					+ e.getMessage());
		} catch (DecodingException e) {
			throw new ApiException(
				"Unable to decode rm call output. DecodingException: " + e.getMessage());
		}
	}
}
