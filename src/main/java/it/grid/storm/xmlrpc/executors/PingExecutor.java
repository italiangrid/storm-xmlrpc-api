...
public class PingExecutor {
	...
	private static final Map<String, Object> DEFAULT_PARAMETERS = PingEncoder
		.getInstance().encode();

	public static PingOutputData execute(synchcall storm, String userDN,
		List<String> userFQANS) throws ApiException, IllegalArgumentException {

		if (storm == null || userDN == null || userFQANS == null
			|| userFQANS.isEmpty()) {
			throw new IllegalArgumentException(
				"Unable to call ping command. Received null arguments: storm="
		...
		try {
			...
			throw new ApiException(
				"Unable to encode Ping parameters. IllegalArgumentException: "
					+ e.getMessage());
		}
	...
	private static PingOutputData doIt(synchcall storm,
		...
		try {
			...
			throw new ApiException("Unable to perform ping call. XmlRpcFault: "
				+ e.getMessage());
		}
		try {
			...
			throw new ApiException(
				"Unable to decode ping call output. IllegalArgumentException: "
					+ e.getMessage());
		} catch (DecodingException e) {
			throw new ApiException(
				"Unable to decode ping call output. DecodingException: "
					+ e.getMessage());
		}
	}
