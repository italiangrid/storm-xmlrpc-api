 ...
 */
public class FileTransferOutputData extends RequestOutputData implements OutputData
{
    ...
    public FileTransferOutputData(String surl, String turl, TReturnStatus status, TRequestToken token)
        throws IllegalArgumentException
    {
        super(status);
        if (surl == null || surl.trim().isEmpty() || turl == null || turl.trim().isEmpty() || token == null
                || token.getValue() == null || token.getValue().trim().isEmpty())
        {
            throw new IllegalArgumentException("Unable to create the object, received null parameters: surl="
                    + surl + " turl=" + turl + " token=" + token);
        }
        this.surl = surl;
        this.turl = turl;
        this.token = token;
