...
public class PtGDecoder implements OutputDecoder
{
    ...
    public PtGOutputData decode(Map<String, Object> output) throws DecodingException, IllegalArgumentException
    {
        if (output == null)
        {
        ...
        if(surl == null)
        {
            throw new DecodingException("Unable to decode the SURL. Null " + XmlRpcParameters.SURL_KEY + " attribute");
        }
        String turl = (String) output.get(XmlRpcParameters.TURL_KEY);
        if(turl == null)
        {
            throw new DecodingException("Unable to decode the TURL. Null " + XmlRpcParameters.TURL_KEY + " attribute");
        }
        TRequestToken token;
        try
        {
        ...
        {
            throw new DecodingException("Unable to decode the token. IllegalArgumentException : "
                    + e.getMessage());
        }
        ...
        if (output.containsKey(TSizeInBytes.PNAME_SIZE)) 
        {
            ...
            {
                throw new DecodingException("Unable to decode the output. Null " + TSizeInBytes.PNAME_SIZE + " attribute");
            }
        ...
        if (output.containsKey(TLifeTimeInSeconds.PNAME_PINLIFETIME)) 
        {
            ...
            {
                throw new DecodingException("Unable to decode the output. Null " + TLifeTimeInSeconds.PNAME_PINLIFETIME + " attribute");
            }
            data.setPinLifetime(TLifeTimeInSeconds.decode(output, TLifeTimeInSeconds.PNAME_PINLIFETIME));
        }
        return data;
    }
