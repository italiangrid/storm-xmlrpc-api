...
public class SurlArrayStatusDecoder implements OutputDecoder
{
    ...
    public SurlArrayRequestOutputData decode(Map<String, Object> output) throws IllegalArgumentException, DecodingException
    {
        if (output == null)
        {
        ...
        List<Map<String,Object>> surlSatuses = (List<Map<String, Object>>) output.get(XmlRpcParameters.ARRAYOF_FILE_STATUSES_KEY);
        ArrayList<SurlStatus> statuses = new ArrayList<SurlStatus>(surlSatuses.size()); 
        ...
        for(Map<String,Object> surlSatus : surlSatuses)
        {
            ...
            if(decodedSurl == null || surlSatus.get(TReturnStatus.PNAME_STATUS) == null)
            {
                throw new DecodingException("Unable to decode the surl status output. Missing mandatory fields: surlSatus="
                                                   + surlSatus);
            }
            
            try
            {
                decodedFileStatus = TReturnStatus.decode((Map<String, Object>) surlSatus.get(TReturnStatus.PNAME_STATUS));    
            } catch(IllegalArgumentException e)
            {
                throw new DecodingException("Unable to decode the surl status Return status \'" + surlSatus.get(TReturnStatus.PNAME_STATUS) + "\'. IllegalArgumentException: "
                                            + e.getMessage());
            }
