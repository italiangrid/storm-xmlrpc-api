...
public class LsDecoder implements OutputDecoder {
	...
	public LsOutputData decode(Map<String, Object> output)
	...
	private SurlInfo decodeSurlInfo(Map<String, Object> surlInfo)
		...
		if (surlInfo == null) {
			throw new IllegalArgumentException(
				"Unable to perform decoding, received null argument: surlInfo="
					+ surlInfo);
		}
		if (surlInfo.get(XmlRpcParameters.LS_FILE_PATH_KEY) == null
			|| surlInfo.get(TReturnStatus.PNAME_STATUS) == null) {
			throw new DecodingException(
				"Unable to decode the output. Missing mandatory fields: surlInfo="
					+ surlInfo);
		}
		String stfn = (String) surlInfo.get(XmlRpcParameters.LS_FILE_PATH_KEY);
		TReturnStatus status;
		try {
			status = TReturnStatus.decode((Map<String, Object>) surlInfo
				.get(TReturnStatus.PNAME_STATUS));
		} catch (IllegalArgumentException e) {
			throw new DecodingException("Unable to decode the Return status \'"
				+ surlInfo.get(TReturnStatus.PNAME_STATUS)
				+ "\'. IllegalArgumentException: " + e.getMessage());
		}
		...
		if (surlInfo.containsKey(TSizeInBytes.PNAME_SIZE)) {
			if (surlInfo.get(TSizeInBytes.PNAME_SIZE) == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
		...
		if (surlInfo.containsKey(XmlRpcParameters.LS_FILE_CREATION_TIME_KEY)) {
			String creationTime = (String) surlInfo
				.get(XmlRpcParameters.LS_FILE_CREATION_TIME_KEY);
			if (creationTime == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ XmlRpcParameters.LS_FILE_CREATION_TIME_KEY + " attribute");
			}
			try {
				info.setCreationTime(dateFormat.parse(creationTime));
			} catch (ParseException e) {
				throw new DecodingException(
					"Unable to decode the CreationTime. ParseException: "
						+ e.getMessage());
			}
		}
		if (surlInfo.containsKey(XmlRpcParameters.LS_FILE_MODIFICATION_TIME_KEY)) {
			String modificationTime = (String) surlInfo
				.get(XmlRpcParameters.LS_FILE_MODIFICATION_TIME_KEY);
			if (modificationTime == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ XmlRpcParameters.LS_FILE_MODIFICATION_TIME_KEY + " attribute");
			}
			try {
				info.setModificationTime(dateFormat.parse(modificationTime));
			} catch (ParseException e) {
				throw new DecodingException(
					"Unable to decode the ModificationTime. ParseException: "
						+ e.getMessage());
			}
		}
		if (surlInfo.containsKey(TFileStorageType.PNAME_FILESTORAGETYPE)) {
			if (surlInfo.get(TFileStorageType.PNAME_FILESTORAGETYPE) == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ TFileStorageType.PNAME_FILESTORAGETYPE + " attribute");
			}
			info.setStorageType(TFileStorageType.decode(surlInfo,
		...
		if (surlInfo.containsKey(TRetentionPolicyInfo.PNAME_retentionPolicyInfo)) {
			if (surlInfo.get(TRetentionPolicyInfo.PNAME_retentionPolicyInfo) == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ TRetentionPolicyInfo.PNAME_retentionPolicyInfo + " attribute");
			}
			info.setRetentionPolicy(TRetentionPolicyInfo.decode(surlInfo,
		...
		if (surlInfo.containsKey(TFileLocality.PNAME_FILELOCALITY)) {
			if (surlInfo.get(TFileLocality.PNAME_FILELOCALITY) == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ TFileLocality.PNAME_FILELOCALITY + " attribute");
			}
			info.setLocality(TFileLocality.decode(surlInfo,
				TFileLocality.PNAME_FILELOCALITY));
		}
		if (surlInfo.containsKey(ArrayOfTSpaceToken.PNAME_ARRAYOFSPACETOKENS)) {
			if (surlInfo.get(ArrayOfTSpaceToken.PNAME_ARRAYOFSPACETOKENS) == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ ArrayOfTSpaceToken.PNAME_ARRAYOFSPACETOKENS + " attribute");
			}
			info.setSpaceTokenArray(ArrayOfTSpaceToken.decode(surlInfo,
		...
		if (surlInfo.containsKey(TFileType.PNAME_TYPE)) {
			if (surlInfo.get(TFileType.PNAME_TYPE) == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ TFileType.PNAME_TYPE + " attribute");
			}
		...
		if (surlInfo.containsKey(TLifeTimeInSeconds.PNAME_LIFETIMEASSIGNED)) {
			if (surlInfo.get(TLifeTimeInSeconds.PNAME_LIFETIMEASSIGNED) == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ TLifeTimeInSeconds.PNAME_LIFETIMEASSIGNED + " attribute");
			}
			info.setLifetime(TLifeTimeInSeconds.decode(surlInfo,
		...
		if (surlInfo.containsKey(TLifeTimeInSeconds.PNAME_LIFETIMELEFT)) {
			if (surlInfo.get(TLifeTimeInSeconds.PNAME_LIFETIMELEFT) == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ TLifeTimeInSeconds.PNAME_LIFETIMELEFT + " attribute");
			}
			info.setLifetimeLeft(TLifeTimeInSeconds.decode(surlInfo,
				TLifeTimeInSeconds.PNAME_LIFETIMELEFT));
		}
		if (surlInfo.containsKey(TUserPermission.PNAME_OWNERPERMISSION)) {
			if (surlInfo.get(TUserPermission.PNAME_OWNERPERMISSION) == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ TUserPermission.PNAME_OWNERPERMISSION + " attribute");
			}
			info.setUserPermission(TUserPermission.decode(surlInfo,
				TUserPermission.PNAME_OWNERPERMISSION));
		}
		if (surlInfo.containsKey(TGroupPermission.PNAME_GROUPPERMISSION)) {
			if (surlInfo.get(TGroupPermission.PNAME_GROUPPERMISSION) == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ TGroupPermission.PNAME_GROUPPERMISSION + " attribute");
			}
			info.setGroupPermission(TGroupPermission.decode(surlInfo,
				TGroupPermission.PNAME_GROUPPERMISSION));
		}
		if (surlInfo.containsKey(TPermissionMode.PNAME_OTHERPERMISSION)) {
			if (surlInfo.get(TPermissionMode.PNAME_OTHERPERMISSION) == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ TPermissionMode.PNAME_OTHERPERMISSION + " attribute");
			}
			info.setOtherPermission(TPermissionMode.decode(surlInfo,
				TPermissionMode.PNAME_OTHERPERMISSION));
		}
		if (surlInfo.containsKey(TCheckSumType.PNAME_CHECKSUMTYPE)) {
			if (surlInfo.get(TCheckSumType.PNAME_CHECKSUMTYPE) == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ TCheckSumType.PNAME_CHECKSUMTYPE + " attribute");
			}
			info.setCheckSumType(TCheckSumType.decode(surlInfo,
				TCheckSumType.PNAME_CHECKSUMTYPE));
		}
		if (surlInfo.containsKey(TCheckSumValue.PNAME_CHECKSUMVALUE)) {
			if (surlInfo.get(TCheckSumValue.PNAME_CHECKSUMVALUE) == null) {
				throw new DecodingException("Unable to decode the surlInfo. Null "
					+ TCheckSumValue.PNAME_CHECKSUMVALUE + " attribute");
			}
			info.setCheckSumValue(TCheckSumValue.decode(surlInfo,
				TCheckSumValue.PNAME_CHECKSUMVALUE));
		}

		Collection<Map<String, Object>> subpathsInfo = (Collection<Map<String, Object>>) surlInfo
			.get(XmlRpcParameters.LS_SUBPATH_DETAILS_KEY);
